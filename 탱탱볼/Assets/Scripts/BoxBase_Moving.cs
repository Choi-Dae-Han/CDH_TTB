    ...
    private void OnTriggerEnter2D(Collider2D CrashObj)
    {
        if (CrashObj.gameObject.CompareTag("Player"))
        {
            ...
            if (fDistTop <= fDistLeft && fDistTop <= fDistRight) // 위 충돌
            {
                ...
                if (!bArrowBox)
                {
                    AddVelo(ref ball.BottomCrashed, ref ball.fYVelocity, ball.fBouncePower + fAddBouncePower + fYVelocity);
            ...
            {
                AddVelo(ref ball.TopCrashed, ref ball.fYVelocity, -ball.fHitPower + fYVelocity, SE_Wall);
                if (ball.BottomCrashed) ball.ChangeState(Ball.STATE.DEAD);
            }
            else if (fDistLeft <= fDistTop && fDistLeft <= fDistBottom)
            {
                AddVelo(ref ball.LeftCrashed, ref ball.fXVelocity, -ball.fHitPower + fXVelocity, SE_Wall);
                if (ball.RightCrashed) ball.ChangeState(Ball.STATE.DEAD);
            }
            else
            {
                AddVelo(ref ball.RightCrashed, ref ball.fXVelocity, ball.fHitPower + fXVelocity, SE_Wall);
